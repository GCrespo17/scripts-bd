...
/**
 * PROCEDIMIENTO: SP_FINALIZAR_EXPOSICION
 * PROPÓSITO: Finaliza una exposición activa, actualizando su fecha de fin y,
 * de forma mandatoria, registrando la cantidad total de visitantes.
 * Esta mejora asegura la integridad de los datos para los cálculos de ingresos.
 * PARÁMETROS:
 * p_id_expo          (IN) - ID de la exposición a finalizar.
 * p_id_museo         (IN) - ID del museo al que pertenece la exposición.
 * p_cant_visitantes  (IN) - Cantidad total de visitantes que asistieron al evento.
 */
CREATE OR REPLACE PROCEDURE SP_FINALIZAR_EXPOSICION (
    ...
    p_cant_visitantes   IN EXPOSICIONES_EVENTOS.cant_visitantes%TYPE -- Parámetro nuevo y clave
)
AS
BEGIN
    -- Se valida que la cantidad de visitantes sea un número no negativo.
    IF p_cant_visitantes IS NULL OR p_cant_visitantes < 0 THEN
        RAISE_APPLICATION_ERROR(-20014, 'Error: La cantidad de visitantes debe ser un número igual o mayor a cero.');
    END IF;

    UPDATE EXPOSICIONES_EVENTOS
    SET fecha_fin = SYSDATE,
        cant_visitantes = p_cant_visitantes -- Se registra la cantidad de visitantes
    WHERE id_expo = p_id_expo
      AND id_museo = p_id_museo
      AND fecha_fin >= SYSDATE; -- Solo se puede finalizar una exposición activa

    IF SQL%NOTFOUND THEN
        RAISE_APPLICATION_ERROR(-20013, 'Error: No se encontró una exposición activa con ID ' || p_id_expo || ' en el museo ID ' || p_id_museo || '.');
    END IF;

EXCEPTION
    WHEN OTHERS THEN
        RAISE; -- Propaga cualquier otro error inesperado.
...
BEGIN
    ...
    WHERE id_museo = p_id_museo
      ...
      AND costo_persona > 0 
      AND cant_visitantes > 0; -- Asegura que no se multipliquen valores nulos o cero
...
IS
    ...
    v_museo_existe     NUMBER;
BEGIN
    -- ========= PASO 1: CENTRALIZAR VALIDACIÓN DE PARÁMETROS =========
    BEGIN
    ...
    -- ========= PASO 2: ORQUESTAR CÁLCULO Y CONSOLIDAR RESULTADOS =========
    v_ingresos_totales := 
...
-- Esta seccion contiene los INSERT para poblar las tablas
-- del proyecto.
-- -----------------------------------------------------------------------------
